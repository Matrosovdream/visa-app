<template>
    <div>
        <PageHeader title="Product extras" subtitle="Add-on services attached to products.">
            <template #actions>
                <button type="button" class="adm-btn adm-btn--primary" @click="openCreate">+ New extra</button>
            </template>
        </PageHeader>

        <DataTable :rows="rows" :columns="columns" :loading="loading" :error="error"
            search-placeholder="Search extras…" :search-keys="['name', 'product.name']">
            ...
            <transition name="adm-modal">
                <div v-if="form" class="adm-modal" @mousedown.self="close">
                    <div class="adm-modal__panel" style="max-width: 520px;">
                        <h3 class="adm-modal__title">{{ form.id ? 'Edit extra' : 'New extra' }}</h3>
...
async function loadAll() {
    loading.value = true;
    try {
        const [r, p] = await Promise.all([api.get('/extras'), api.get('/products')]);
        rows.value = r.data.data ?? [];
        products.value = p.data.data ?? [];
    } catch (e) {
        error.value = 'Could not load extras.';
    } finally {
        loading.value = false;
    }
}
...
async function save() {
    ...
    try {
        ...
        delete payload.id;
        if (form.value.id) {
            const { data } = await api.put(`/extras/${form.value.id}`, payload);
        ...
        else toast.error(e.response?.data?.message || 'Could not save extra.');
    } finally {
        saving.value = false;
    }
}
...
async function askDelete(row) {
    const ok = await confirm.ask({
        title: `Delete extra "${row.name}"?`,
        confirmLabel: 'Delete', danger: true,
    });
    if (!ok) return;
    try {
        await api.delete(`/extras/${row.id}`);
        rows.value = rows.value.filter((r) => r.id !== row.id);
        toast.success('Extra deleted.');
    } catch (e) {
        toast.error(e.response?.data?.message || 'Could not delete extra.');
    }
}
